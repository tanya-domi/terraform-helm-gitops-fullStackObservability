...
resource "google_container_cluster" "primary" {
  ...
  # PRODUCTION SECURITY FIX: Blocks legacy v1 Compute Engine metadata endpoints
  metadata {
...
# ==============================================================================
# Node Pool: Application Workloads
# ==============================================================================
...
resource "google_container_node_pool" "app_nodes" {
  ...
  node_config {
    ...
    # Restricts pod metadata access exclusively through Workload Identity proxies
    workload_metadata_config { 
      mode = "GKE_METADATA" 
    }
...
# ==============================================================================
# Node Pool: Dedicated Observability Workloads (Thanos, Tempo, Prometheus, Grafana)
# ==============================================================================
# tfsec:ignore:google-gke-node-metadata-security
resource "google_container_node_pool" "monitor_nodes" {
  project    = var.project_id
  ...
  node_config {
    machine_type = "e2-standard-4" # Heavier compute slice for observability engines
    disk_size_gb = 50
    disk_type    = "pd-balanced"
    oauth_scopes = ["https://www.googleapis.com/auth/cloud-platform"]
